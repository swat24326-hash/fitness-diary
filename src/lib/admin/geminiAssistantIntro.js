...
function buildStandardIntro(ctx, tail, coverage) {
  ...
  return (
    ...
    `Могу: план продаж (уровни 1–3), ПНК, лучший день по прибыли, НК/ДК/УК, сравнение с прошлым месяцем, расхождение отчёт/планшеты, ФОТ и маржа. Менеджера не заменяю, цифры не придумываю.${tail}${coverage} ` +
...
function buildCapabilitiesIntro(ctx, tail) {
  ...
  return (
    `${name} ${scope}: ` +
    `план и выручка (уровни 1–3), ПНК, лучший день, НК/ДК/УК, главный косяк, динамика к прошлому месяцу, FIT-CITY vs отчёт менеджера, ФОТ и чистая прибыль. ` +
