...
function buildStandardIntro(ctx, tail, coverage) {
  ...
  return (
    ...
    `Могу: план продаж, где просели, сравнение с прошлым месяцем, расхождение отчёт/планшеты, ФОТ и маржа. Менеджера не заменяю, цифры не придумываю.${tail}${coverage} ` +
...
function buildCapabilitiesIntro(ctx, tail) {
  ...
  return (
    `${name} ${scope}: ` +
    `план и выручка, главный косяк в цифрах, динамика к прошлому месяцу, FIT-CITY vs отчёт менеджера, ФОТ и чистая прибыль. ` +
...
function buildSourcesIntro(ctx) {
  ...
  return (
    ...
    `**Отчёт менеджера** — главный источник: прибыль НК/ДК/УК, план, матрица ПЗ/ТЗ/АЗ, тренировки всего зала. ` +
...
function buildDeepIntro(ctx, coverage) {
  ...
  return (
    ...
    `план = profit_total / plan_total; coverage = дни с отчётом / дни месяца; gap = manager_report_total − fit_city_tablets_only. ` +
