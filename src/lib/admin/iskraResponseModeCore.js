...
export function buildIskraResponseFormatRule(mode) {
  ...
  if (mode === 'standard') {
    return [
      ...
      'Первая строка — главный вывод одним предложением (без «Данные приняты» и без «На связи» в начале).',
      'Дальше 2–4 коротких пункта с новой строки (1. или —), не длиннее 12 слов каждый.',
      'ЗАПРЕЩЕНО: шаблон «Факты: / Вывод: / Рекомендации:», markdown **, простыня в одном абзаце.',
      'Цифры только из данных приложения. «На связи» — только в конце короткого ответа или не использовать.',
    ].join('\n')
  }
