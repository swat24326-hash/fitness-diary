    ...
    try {
      ...
      if (data.ok && data?.kpi) {
        return {
          ...
          adviceOutcomes: Array.isArray(data.advice_outcomes) ? data.advice_outcomes : [],
  ...
  const res = await fetchWithAppTimeout(
    `${apiOrigin()}/api/admin-data?action=gemini-analytics`,
    {
      ...
      body: JSON.stringify({
        ...
        input_channel: opts.inputChannel === 'voice' ? 'voice' : undefined,
        coach_quality_brief: opts.coachQualityBrief ?? undefined,
      }),
    },
    GEMINI_REQUEST_TIMEOUT_MS,
  )
