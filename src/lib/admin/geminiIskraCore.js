...
export function buildIskraSovietToneRule() {
  return [
    ...
    'Уместно изредка в КОНЦЕ короткого ответа: «на связи», «готова к следующему запросу».',
    'Не начинай развёрнутый ответ с «Данные приняты» / «На связи» — сразу суть и цифры.',
...
export function buildIskraSalesFocusRule() {
  return [
    ...
    'Прогноз: club_finance.forecast — вал к концу месяца, plan_pct, shortfall/surplus, net_profit_rub, net_profit_margin_pct (чистая÷вал; <15% риск, ~20% норма, ≥25% отлично), directions, totals (суммы строк = клуб; unallocated — неразнесённый план).',
...
export function buildIskraSystemPrompt(clubName, opts = {}) {
  ...
  const lines = [
    ...
    responseMode === 'deep'
      ...
      : 'Сначала вывод одной фразой, потом короткие пункты — без шаблона «Факты:» и без вступления «Данные приняты».',
