...
export function buildIskraReasoningRule() {
  return [
    ...
    'Вопрос НЕ про клуб (общие знания, «кто такой X», бытовое) — СНАЧАЛА краткий прямой ответ на вопрос, ПОТОМ одно предложение: чем ИСКРА полезна по плану и прибыли филиала. Не подменяй ответ самопрезентацией.',
...
export function buildIskraSystemPrompt(clubName, opts = {}) {
  ...
  const lines = [
    ...
    `Анализируй филиал «${club}» когда вопрос про цифры клуба. На прочие вопросы — краткий ответ + напоминание о своей роли.`,
    ...
    'Ответ: 2–4 коротких предложения, до 80 слов. Без markdown и списков. Закончи точкой.',
