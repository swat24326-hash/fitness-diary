...
**План ЗП тренеров:** пороги тренировок месяца (ур. 2 / 3) — **Структура → План ЗП**; ставки ₽ — **Типы абон.** (`trainer_pay_l1/l2/l3`); кабинет сотрудника (с планом / без плана + ±₽ за тренировку) — **Структура → Тренеры → кабинет** (иконка кошелька). Расчёт ЗП: уровень из порогов+профиля × ставка типа + adjustment (не ниже 0).  
