...
Короткий вход в корне: [`AGENTS.md`](../AGENTS.md). Экономия лимитов: [CURSOR_LIMITS.md](./CURSOR_LIMITS.md).
...
|-----|------|------------------|
| 0 | [`AGENTS.md`](../AGENTS.md) | Ещё короче, если уже знаешь тип задачи |
...
Не читать подряд все 50+ файлов `docs/` — только ветка задачи. Экономия **без потери политики**: [CURSOR_LIMITS.md](./CURSOR_LIMITS.md).
...
Политики в `.cursor/rules/` **действуют все**. Экономия — узкий вход (`AGENTS.md`), 1–2 doc по ветке задачи и то, что два самых больших правила подключаются **по типу задачи**, а не в каждый запрос:

- `fitness-diary-features.mdc` — открыть до плана и кода на новой фиче или развитии функции;
- `fitness-diary-north-star-lead.mdc` — открыть при споре о приоритете, вопросе «что дальше», предложении оплат или переезда РФ.

Жёсткие рамки (цель, фазы, «оплаты после R3+») остаются в `fitness-diary-architecture.mdc` — они грузятся всегда.
...
Полный список — [PROJECT_HANDOFF_FOR_AI.md](./PROJECT_HANDOFF_FOR_AI.md) §15. Экономия без минусов: [CURSOR_LIMITS.md](./CURSOR_LIMITS.md).
...
Часть проверок идёт сама: хук `stop-check` линтует изменённые файлы и напоминает про `qa:critical`, `guard-shell` спрашивает только опасные команды (reset, миграции, `rm`) — [HOOKS.md](./HOOKS.md). «Заливай» в чате = коммит + push + прод без вопросов по шагам. Проверки всё равно запускать самому.
...
| Вопрос | Ответ |
|--------|--------|
| Короткий вход | [`AGENTS.md`](../AGENTS.md) |
| Экономия Cursor | [CURSOR_LIMITS.md](./CURSOR_LIMITS.md) |
