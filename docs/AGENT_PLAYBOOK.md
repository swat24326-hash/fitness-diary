...
Часть проверок идёт сама: хук `stop-check` линтует изменённые файлы и напоминает про `qa:critical`, `guard-shell` требует подтверждения на push/деплой — [HOOKS.md](./HOOKS.md). Это страховка, проверки всё равно запускать самому.
