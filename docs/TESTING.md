...
| Жизнь по направлениям | `verify-client-hall-lifecycle.mjs` (A–G); **списки** — `verify-admin-clients-list-lifecycle.mjs` (ПЗ→АЗ); migrate `npm run db:migrate:client-hall-lifecycle -- --linked`; [CLIENT_HALL_LIFECYCLE.md](./CLIENT_HALL_LIFECYCLE.md) |
