...
| `client-retention` | admin / trainer / **supervisor** (свой клуб; у тренера — только свой `trainer_id`) | KPI удержания tablet-клиентов: M+3, продления, архив, mix причин, M+3 по тренерам. Параметры: `club_id`, `date_from`, `date_to`, опц. `trainer_id`. См. `docs/CLIENT_RETENTION.md` |
| `client-attendance` | admin / trainer / **supervisor** (как retention) | Посещаемость ПЗ: **средняя трен./нед.** (2 знака), % без выпадения, структура ритма, byTrainer. Окно = `date_from`…`date_to` (как период сводки; `date_to` clamp сегодня; без `date_from` — последние 30 дн.). Визиты по `client_id` пула (не по trainer_id тренировки). Параметры: `club_id`, `date_from`, `date_to`, опц. `trainer_id` |
