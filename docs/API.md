...
| `client-attendance` | admin / trainer / **supervisor** (как retention) | Посещаемость ПЗ: **средняя трен./нед.** (2 знака), % без выпадения, структура ритма, byTrainer. Окно 30 дн. на `date_to` (clamp сегодня). Визиты по `client_id` пула (не по trainer_id тренировки). Параметры: `club_id`, `date_from`, `date_to`, опц. `trainer_id` |
