...
3d. **Уход на главную / возврат в черновик:** ввод сразу в live-ref; flush session+durable при уходе; при открытии `pickTrainingDraftRestore` выбирает самый полный снимок (IDB / durable / session) в окне «богатства». **Сплит вкладок (два клиента):** перед сменой вкладки снимок из `liveDraftRef` (gate может отставать); silent persist не пишет в IDB, пока `meta.trainingId` не совпал с URL. Verify: `scripts/verify-training-draft-restore.mjs`, `scripts/verify-training-draft-tab-switch.mjs`.
...
10. **Prefetch тренера (online, не блокирует UI):** при открытии формы тренировки / карточки клиента — фоновый `ensureClientTrainingsCached` + `refreshMembershipsForStats` (`trainingClientPrefetch.js`, TTL дневника 90 с). Pull-guard pending не ослабляем. Ключ дедупликации снимается, если refresh не отработал (cooldown / офлайн / ошибка) — иначе следующий заход в черновик уже не пробует.
10a. **Абонемент не доехал на планшет:** плитка «Трен.» и списание при «Закончить» независимо читают `listMemberships` из IDB. Пусто и online → **точечная** догрузка `ensureTrainerMembershipsFresh` (ждём результат, `force: true` мимо cooldown), потом пересчёт плитки; перед отказом в списании — одна такая попытка. Офлайн отказываем сразу, сеть не ждём. Решения — `trainingMembershipEnsureCore.js`, verify: `scripts/verify-training-membership-ensure.mjs`. Симптом-первоисточник: прочерки в плитках, тренировка не закрывалась (INC-2026-09-07-02).
...
12. **Таймауты pull при ручном Sync:** UI-клики — `ADMIN_FETCH_TIMEOUT_MS` (5 с). **Sync-pull** — `SYNC_PULL_FETCH_TIMEOUT_MS` (45 с): `trainer-pull`, полный справочник упражнений (`force: true`), **челленджи / типы абонементов / питание / шаблоны ДЗ** (`pullReferenceData.js` → `REF_API_TIMEOUT`); потолок всего ручного Sync — `MANUAL_SYNC_GUARD_MS` в `useHeaderSync.js`. Иначе cold start Vercel давал «Таймаут связи» при живом сервере (в т.ч. только на челленджах при пустой очереди). Челленджи: при сбое API — fallback на прямой Supabase, как у типов абонементов.
12b. **«Только на устройстве» после Sync (тренировка):** insert уже в облаке → 409 со старым `updated_at` → локаль казалась новее → раньше `applyPushRecordToLocal` выходил без `synced: true` → вечный local-only. Сейчас `resolveAfterPushAck`: та же ревизия, что ушла в push → пометить synced; правка в полёте → `__sync: update`. Verify: `verify-sync-unsynced.mjs`.
12c. **Планшет «Завершена», компьютер/ЗП — черновик:** гонка автосейва draft и «Закончить»: поздний HTTP draft мог перезаписать `completed` в облаке (select→update TOCTOU). Защита: draft-update только при `status=draft` в БД; устаревший draft поверх completed не пишется; batch merge и push-ack не откатывают completed; при pull/hydrate local completed vs cloud draft — requeue на повторный push (`shouldRequeueLocalCompletedOverCloudDraft`). Verify: `verify-training-persist-status`, `verify-sync-offline`, `verify-sync-pull-merge`, `verify-sync-unsynced`. INC-2026-09-09-01.
