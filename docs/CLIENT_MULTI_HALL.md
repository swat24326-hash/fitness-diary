...
**Плитки на экране Клиенты** — один контур с списком (`adminClientsBrowseFilterCore.js`): вкладка ПЗ/ТЗ/АЗ + воронка; chip = длина списка (holding на «Не назначен» не вырезается). **Сводка дня** = те же counts, что Clients → ПЗ. **Commercial census** — только статистика, не плитки.
