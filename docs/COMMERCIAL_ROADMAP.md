...
- ⏸ **Админ online-first** (запись в API без обязательного Sync; тренер офлайн как сейчас) — [SYNC.md](./SYNC.md).
...
| **Управляющий** (supervisor, один на клуб) | ✅ в проде (`/club`): ≈ админ клуба + «Настройки» (Max/SMS); зал — отдельный тренер. [CLUB_SUPERVISOR.md](./CLUB_SUPERVISOR.md) |
