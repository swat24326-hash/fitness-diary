...
| Роль | Где |
|------|-----|
| Админ | `/admin/sales?tab=price` — **ПЗ \| ТЗ \| АЗ**, любой клуб |
| Менеджер | `/sales?tab=price` — то же для своего клуба |
| Управляющий | вкладка «Прайс» своего клуба; **правит наравне с менеджером** (проверка на сервере — `priceListAccessCore.js`, кейсы в `scripts/verify-price-list.mjs`) |
