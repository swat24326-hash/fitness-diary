...
Скрипт сверяет md с кодом: у каждой статьи из JS есть свой `<id>.md`, заголовок совпадает с `title`, число шагов одинаковое, лишних md нет. Так «поправил только md» больше не проходит тихо.
