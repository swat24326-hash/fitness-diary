...
**Важно для того, кто будет включать роли:** сейчас `mapAppRoleToAdvisorRole` возвращает `app_admin` для любой роли (ветки supervisor и curator закомментированы), поэтому `filterSnapshotForAdvisorRole` фактически ничего не скрывает. Данные не утекают только потому, что чат ИСКРЫ закрыт `requireAdmin`. Открывать чат другой роли можно **лишь вместе** с включением маппинга и verify на фильтр — иначе управляющий сразу увидит финансы и ЗП.
