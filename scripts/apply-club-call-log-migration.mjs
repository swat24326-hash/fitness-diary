/**
 * Журнал звонков клуба: таблица club_call_log + RLS.
 *
...
const MIGRATION = 'supabase/migrations/20260813220000_club_call_log.sql'
const VERIFY_FILE = 'scripts/.tmp-verify-club-call-log.sql'
const VERIFY_SQL =
  "select table_name from information_schema.tables where table_schema = 'public' and table_name = 'club_call_log';\n"
...
if (useLinked) {
  ...
  if (!password) {
    fail('Задайте SUPABASE_DB_PASSWORD или: npm run db:migrate:club-call-log -- --linked')
  }
  const ref =
...
try {
  run('verify club_call_log exists', ['db', 'query', ...dbArgs, '--file', VERIFY_FILE, '--yes'])
} finally {
  try {
...
console.log('\nclub_call_log migration applied.')
