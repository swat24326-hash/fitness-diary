/**
 * Применить миграцию лояльности ПЗ.
 *   npm run db:migrate:loyalty -- --linked
 */
import { spawnSync } from 'node:child_process'
import { existsSync, readFileSync, writeFileSync, unlinkSync } from 'node:fs'
import { resolve } from 'node:path'

const MIGRATION = 'supabase/migrations/20260818235900_loyalty.sql'
const VERIFY_FILE = 'scripts/.tmp-verify-loyalty.sql'
const VERIFY_SQL =
  "select table_name from information_schema.tables where table_schema = 'public' and table_name in ('club_loyalty_settings','loyalty_ledger') order by 1;\n"
...
if (useLinked) {
  ...
  if (!password) {
    fail('Задайте SUPABASE_DB_PASSWORD или: npm run db:migrate:loyalty -- --linked')
  }
  const ref =
...
try {
  run('verify loyalty tables', ['db', 'query', ...dbArgs, '--file', VERIFY_FILE, '--yes'])
} finally {
  try {
