/**
 * Снимки правил ЗП по календарному месяцу.
 *
 *   npm run db:migrate:trainer-pay-month-snapshots -- --linked
 */
...
const MIGRATION = 'supabase/migrations/20260809120000_club_trainer_pay_month_snapshots.sql'
const VERIFY_SQL = `select table_name
from information_schema.tables
where table_schema = 'public'
  and table_name = 'club_trainer_pay_month_snapshots';
`
...
function withVerifyFile(fn) {
  const verifyPath = resolve(tmpdir(), `fd-verify-pay-month-snap-${Date.now()}.sql`)
...
console.log('\nМиграция club_trainer_pay_month_snapshots применена.')
