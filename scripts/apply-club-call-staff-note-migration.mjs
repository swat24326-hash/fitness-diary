/**
 * Миграция пометки к звонку: club_call_log.staff_note.
 *
 *   npm run db:migrate:club-call-staff-note -- --linked
 */
import { spawnSync } from 'node:child_process'
import { existsSync, readFileSync, writeFileSync, unlinkSync } from 'node:fs'
import { resolve } from 'node:path'

const MIGRATION = 'supabase/migrations/20260815010000_club_call_log_staff_note.sql'
const VERIFY_FILE = 'scripts/.tmp-verify-club-call-staff-note.sql'
const VERIFY_SQL =
  "select column_name from information_schema.columns where table_schema = 'public' and table_name = 'club_call_log' and column_name = 'staff_note';\n"
...
if (useLinked) {
  ...
  if (!password) {
    fail('Задайте SUPABASE_DB_PASSWORD или: npm run db:migrate:club-call-staff-note -- --linked')
  }
  const ref =
...
try {
  run('verify staff_note column', ['db', 'query', ...dbArgs, '--file', VERIFY_FILE, '--yes'])
} finally {
  try {
...
console.log('\nclub_call_log staff_note migration applied.')
