/**
 * Миграция: clients.expected_return_on (ориентир «Вернётся позже»).
 *
...
const MIGRATION = 'supabase/migrations/20260820140000_clients_expected_return_on.sql'
const VERIFY_FILE = 'scripts/.tmp-verify-clients-expected-return.sql'
const VERIFY_SQL =
  "select column_name from information_schema.columns where table_schema = 'public' and table_name = 'clients' and column_name = 'expected_return_on';\n"
...
if (useLinked) {
  ...
  if (!password) {
    fail('Задайте SUPABASE_DB_PASSWORD или: npm run db:migrate:clients-expected-return -- --linked')
  }
  const ref =
...
run('migrate expected_return_on', ['db', 'query', ...dbArgs, '--file', MIGRATION])

try {
  writeFileSync(resolve(VERIFY_FILE), VERIFY_SQL, 'utf8')
  run('verify column', ['db', 'query', ...dbArgs, '--file', VERIFY_FILE])
} finally {
  try {
...
console.log('\nГотово: clients.expected_return_on')
