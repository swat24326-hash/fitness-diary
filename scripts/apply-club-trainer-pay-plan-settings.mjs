/**
 * Пороги часов плана ЗП тренера (club_trainer_pay_plan_settings).
 *
 *   npm run db:migrate:trainer-pay-plan -- --linked
 */
...
const MIGRATION = 'supabase/migrations/20260808130000_club_trainer_pay_plan_settings.sql'
const VERIFY_SQL = `select table_name
from information_schema.tables
where table_schema = 'public'
  and table_name = 'club_trainer_pay_plan_settings';
`
...
function withVerifyFile(fn) {
  const verifyPath = resolve(tmpdir(), `fd-verify-trainer-pay-plan-${Date.now()}.sql`)
...
console.log('\nМиграция club_trainer_pay_plan_settings применена.')
