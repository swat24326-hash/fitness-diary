/**
 * RLS: менеджер продаж читает membership_types (включая АЗ) + фикс R3+ как АЗ.
 *
 *   npm run db:migrate:membership-types-sm -- --linked
 * или с паролем:
 *   $env:SUPABASE_DB_PASSWORD="…"
 *   npm run db:migrate:membership-types-sm
 */
...
const MIGRATION = 'supabase/migrations/20260729120000_membership_types_sales_manager_read.sql'
const VERIFY_SQL = `select pol.polname
from pg_policy pol
join pg_class cls on cls.oid = pol.polrelid
join pg_namespace nsp on nsp.oid = cls.relnamespace
where nsp.nspname = 'public'
  and cls.relname = 'membership_types'
  and pol.polname = 'fit_membership_types_sales_manager_read';
`
...
function withVerifyFile(fn) {
  const verifyPath = resolve(tmpdir(), `fd-verify-mt-sm-${Date.now()}.sql`)
...
if (useLinked) {
  ...
  withVerifyFile((verifyPath) => {
    run('verify policy (linked)', ['db', 'query', '--linked', '--file', verifyPath, '--yes'])
  })
} else {
  ...
  withVerifyFile((verifyPath) => {
    run('verify policy', ['db', 'query', '--db-url', dbUrl, '--file', verifyPath, '--yes'])
  })
}

console.log('\nМиграция membership_types_sales_manager_read применена.')
