/**
 * Миграция входящих: club_call_log.direction + nullable client_id.
 *
 *   npm run db:migrate:club-call-inbound -- --linked
 */
import { spawnSync } from 'node:child_process'
import { existsSync, readFileSync, writeFileSync, unlinkSync } from 'node:fs'
import { resolve } from 'node:path'

const MIGRATION = 'supabase/migrations/20260816020000_club_call_log_inbound.sql'
const VERIFY_FILE = 'scripts/.tmp-verify-club-call-inbound.sql'
const VERIFY_SQL =
  "select column_name from information_schema.columns where table_schema = 'public' and table_name = 'club_call_log' and column_name in ('direction');\n"
...
if (useLinked) {
  ...
  if (!password) {
    fail('Задайте SUPABASE_DB_PASSWORD или: npm run db:migrate:club-call-inbound -- --linked')
  }
  const ref =
...
try {
  run('verify direction column', ['db', 'query', ...dbArgs, '--file', VERIFY_FILE, '--yes'])
} finally {
  try {
...
console.log('\nclub_call_log inbound migration applied.')
