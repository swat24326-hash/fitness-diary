/**
 * Применить миграцию club_tz_price_lists.
 *   npm run db:migrate:tz-price-list -- --linked
 */
import { spawnSync } from 'node:child_process'
import { existsSync, readFileSync } from 'node:fs'
import { resolve } from 'node:path'

const MIGRATION = 'supabase/migrations/20260731150000_club_tz_price_lists.sql'
const VERIFY_SQL =
  "select table_name from information_schema.tables where table_schema='public' and table_name = 'club_tz_price_lists';"
...
if (useLinked) {
  ...
  run('verify table (linked)', ['db', 'query', '--linked', VERIFY_SQL, '--yes'])
} else {
  ...
  run('verify table', ['db', 'query', '--db-url', dbUrl, VERIFY_SQL, '--yes'])
}

console.log('\nМиграция club_tz_price_lists применена.')
