/**
 * Миграция: club_call_log.staff_note_chip_id + callback_on
 *
 *   npm run db:migrate:club-call-funnel-chips -- --linked
 */
import { spawnSync } from 'node:child_process'
import { existsSync, readFileSync, writeFileSync, unlinkSync } from 'node:fs'
import { resolve } from 'node:path'

const MIGRATION = 'supabase/migrations/20260820160000_club_call_log_funnel_chips.sql'
const VERIFY_FILE = 'scripts/.tmp-verify-club-call-funnel-chips.sql'
const VERIFY_SQL =
  "select column_name from information_schema.columns where table_schema = 'public' and table_name = 'club_call_log' and column_name in ('staff_note_chip_id', 'callback_on') order by 1;\n"
...
if (useLinked) {
  ...
  if (!password) {
    fail('Задайте SUPABASE_DB_PASSWORD или: npm run db:migrate:club-call-funnel-chips -- --linked')
  }
  const ref =
...
run('migrate funnel chips', ['db', 'query', ...dbArgs, '--file', MIGRATION])
...
console.log('\nГотово: club_call_log.staff_note_chip_id / callback_on')
