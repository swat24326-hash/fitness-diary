/**
 * Миграция исхода звонков: outcome / duration_sec / finished_at на club_call_log.
 *
 *   npm run db:migrate:club-call-outcome -- --linked
 */
import { spawnSync } from 'node:child_process'
import { existsSync, readFileSync, writeFileSync, unlinkSync } from 'node:fs'
import { resolve } from 'node:path'

const MIGRATION = 'supabase/migrations/20260814153000_club_call_log_outcome.sql'
const VERIFY_FILE = 'scripts/.tmp-verify-club-call-outcome.sql'
const VERIFY_SQL =
  "select column_name from information_schema.columns where table_schema = 'public' and table_name = 'club_call_log' and column_name in ('outcome','duration_sec','finished_at');\n"
...
if (useLinked) {
  ...
  if (!password) {
    fail('Задайте SUPABASE_DB_PASSWORD или: npm run db:migrate:club-call-outcome -- --linked')
  }
  const ref =
...
try {
  run('verify outcome columns', ['db', 'query', ...dbArgs, '--file', VERIFY_FILE, '--yes'])
} finally {
  try {
...
console.log('\nclub_call_log outcome migration applied.')
